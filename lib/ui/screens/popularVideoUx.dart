...
class _PopularVideoUxState extends State<PopularVideoUx> with TickerProviderStateMixin {
  late AnimationController animationController = AnimationController(vsync: this, duration: Duration(milliseconds: 900));
  ...
  late AnimationController secondCircleAnimationController = AnimationController(vsync: this, duration: Duration(milliseconds: 900));
  ...
  late Tween<double> topVideoCardDetailsScaleTween = Tween<double>(begin: 1.3, end: 1.0);
  ...
  void initState() {
    ...
    for (var i = 0; i < videos.length; i++) {
      videoCardAnimationControllers.add(AnimationController(vsync: this, duration: Duration(milliseconds: videoCardAnimationDurationInMilliseconds)));
      videoCardAnimations.add(Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: videoCardAnimationControllers[i], curve: Curves.easeInOut)));
      videoCardScaleTweens.add(Tween<double>(begin: _calculateBeginVideoCardScale(i), end: _calculateEndVideoCardScale(i)));
      ...
      videoCardleftPositionTweens.add(Tween<double>(begin: -widget.screenSize.width * (0.8) * (1.5), end: widget.screenSize.width * (0.1)));
      videoCardTopPaddingTweens.add(Tween<double>(begin: _calculateVideoCardTopPosition(i), end: _calculateVideoCardTopPosition(i)));

      videoCardDetailsAnimationControllers.add(AnimationController(vsync: this, duration: Duration(milliseconds: videoCardDetailsAnimationDuration)));
      if (i == currentTopVideoCardIndex) {
        videoCardDetailsleftPositionTweens.add(Tween<double>(begin: widget.screenSize.width * (-0.9), end: widget.screenSize.width * (0.15)));
        videoCardDetailsAngleTweens.add(Tween<double>(begin: 0.0, end: 0.0));
        videoCardDetailsAnimations.add(Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: videoCardDetailsAnimationControllers[i], curve: Curves.easeInOut)));
      } else {
        videoCardDetailsleftPositionTweens.add(Tween<double>(begin: widget.screenSize.width * (1.15), end: widget.screenSize.width * (0.15)));
  ...
  double _calculateVideoCardTopPosition(int index) {
    return widget.screenSize.height * (0.5) - widget.screenSize.height * (0.2) - (videos.length - 1 - index) * inBetweenVideoCardTopPadding;
  }
  ...
  Widget _buildVideoCard(int index) {
    return AnimatedBuilder(
        ...
        builder: (context, _) {
          final double scale = videoCardAnimations[index].drive(videoCardScaleTweens[index]).value;
          final double leftPosition = videoCardAnimations[index].drive(videoCardleftPositionTweens[index]).value;
          double opacity = videoCardAnimations[index].drive(videoCardFadeTweens[index]).value;
          final topPosition = videoCardAnimations[index].drive(videoCardTopPaddingTweens[index]).value;
                ...
                child: GestureDetector(
                  onTap: () {
                    ...
                    if (index == currentTopVideoCardIndex) {
                      ...
                      int endCondition = (index >= totalCardDiplayedInScreen) ? (currentTopVideoCardIndex - totalCardDiplayedInScreen) : 0;
                      for (var i = currentTopVideoCardIndex; i >= endCondition; i--) {
                        //only top card will be go up side
                        if (i == currentTopVideoCardIndex) {
                          videoCardTopPaddingTweens[i] = Tween<double>(begin: videoCardTopPaddingTweens[i].end, end: widget.screenSize.height * (-0.4));
                          videoCardScaleTweens[i] = Tween<double>(begin: videoCardScaleTweens[i].end, end: videoCardScaleTweens[i].end);
                          videoCardleftPositionTweens[i] = Tween(begin: videoCardleftPositionTweens[i].end, end: videoCardleftPositionTweens[i].end);
                          videoCardFadeTweens[i] = Tween<double>(begin: 1.0, end: 1.0);
                        } else if (i == endCondition) {
                          videoCardTopPaddingTweens[i] = Tween<double>(begin: videoCardTopPaddingTweens[i].end, end: videoCardTopPaddingTweens[i].end! + inBetweenVideoCardTopPadding);
                          videoCardScaleTweens[i] = Tween<double>(begin: videoCardScaleTweens[i].end, end: videoCardScaleTweens[i].end! + videoCardsScaleDifference);

                          videoCardleftPositionTweens[i] = Tween(begin: videoCardleftPositionTweens[i].end, end: videoCardleftPositionTweens[i].end);
                          if (index >= totalCardDiplayedInScreen) {
                            videoCardFadeTweens[i] = Tween<double>(begin: 0.0, end: 1.0);
                          } else {
                            videoCardFadeTweens[i] = Tween<double>(begin: 1.0, end: 1.0);
                          }
                        } else {
                        ...
                        videoCardAnimationControllers[i] = AnimationController(vsync: this, duration: Duration(milliseconds: videoCardDetailsAnimationDuration));
                        videoCardAnimations[i] = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: videoCardAnimationControllers[i], curve: Curves.easeInOut));
                      }

                      if (endCondition > 0) {
                        //if there is more card behind then change those card's scale and topPadding
                        for (var i = (currentTopVideoCardIndex - totalCardDiplayedInScreen - 1); i >= 0; i--) {
                          videoCardAnimationControllers[i].dispose();
                          videoCardAnimationControllers[i] = AnimationController(vsync: this, duration: Duration(milliseconds: videoCardDetailsAnimationDuration));
                          videoCardAnimations[i] = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: videoCardAnimationControllers[i], curve: Curves.easeInOut));
                          videoCardTopPaddingTweens[i] = Tween<double>(begin: videoCardTopPaddingTweens[i].end, end: videoCardTopPaddingTweens[i].end! + inBetweenVideoCardTopPadding);
                      ...
                      videoCardDetailsAnimationControllers[currentTopVideoCardIndex].dispose();
                      videoCardDetailsAnimationControllers[currentTopVideoCardIndex] = AnimationController(vsync: this, duration: Duration(milliseconds: videoCardDetailsAnimationDuration));
                      videoCardDetailsAnimations[currentTopVideoCardIndex] = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: videoCardDetailsAnimationControllers[currentTopVideoCardIndex], curve: Curves.easeInOut));
                      videoCardDetailsAngleTweens[currentTopVideoCardIndex] = Tween<double>(begin: 0.0, end: -25.0);
                      videoCardDetailsleftPositionTweens[currentTopVideoCardIndex] = Tween<double>(begin: widget.screenSize.width * (0.15), end: widget.screenSize.width * (-0.9));
                      //animate second top card index
                      if (currentTopVideoCardIndex > 0) {
                        videoCardDetailsAnimationControllers[currentTopVideoCardIndex - 1].dispose();
                        videoCardDetailsAnimationControllers[currentTopVideoCardIndex - 1] = AnimationController(vsync: this, duration: Duration(milliseconds: videoCardDetailsAnimationDuration));
                        videoCardDetailsAnimations[currentTopVideoCardIndex - 1] = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: videoCardDetailsAnimationControllers[currentTopVideoCardIndex - 1], curve: Curves.easeInOut));
                        videoCardDetailsAngleTweens[currentTopVideoCardIndex - 1] = Tween<double>(begin: 25.0, end: 0.0);
                        videoCardDetailsleftPositionTweens[currentTopVideoCardIndex - 1] = Tween<double>(begin: widget.screenSize.width * (1.15), end: widget.screenSize.width * (0.15));
                      }
                      ...
                      videoCardDetailsAnimationControllers[currentTopVideoCardIndex].forward();
                      if (currentTopVideoCardIndex > 0) {
                        videoCardDetailsAnimationControllers[currentTopVideoCardIndex - 1].forward();
                      }
  ...
  Widget _buildBottomNavigationContainer() {
    return Positioned(
      ...
      child: SlideTransition(
        position: animation.drive<Offset>(Tween(begin: Offset(0.0, 1.0), end: Offset.zero)),
        child: Container(
  ...
  Widget _buildVideoDetailsContainer(int index) {
    return AnimatedBuilder(
        ...
        builder: (context, _) {
          double leftPosition = videoCardDetailsAnimations[index].drive(videoCardDetailsleftPositionTweens[index]).value;
          double angle = videoCardDetailsAnimations[index].drive(videoCardDetailsAngleTweens[index]).value;
